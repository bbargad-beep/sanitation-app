...
with st.expander("פתח הוראות", expanded=False):
    ...
    <div class="step-card">
      <h4>ספר הכללים — איך בודקים עשרות אלפי תיקונים</h4>
      <p>המערכת מבצעת בדרך כלל כ-48,000 תיקונים אוטומטיים על קובץ שנתי.
      אי אפשר לקרוא 48,000 שורות, ואין בכך צורך: כל התיקונים האלה נוצרים
      על ידי כ-<strong>60 כללים בלבד</strong>.</p>
      <p>ספר הכללים מציג כרטיס לכל כלל, מהגדול לקטן, ובכל כרטיס:
      כמה שורות הכלל נגע בהן, מה בדיוק הוא עושה, ושלוש דוגמאות אמיתיות
      מהנתונים שלך. בכללי מילות מפתח, המילה שהפעילה את הכלל
      <strong>מסומנת בצהוב בתוך התיאור</strong> — כך אפשר לראות את הראיה
      ולא רק לסמוך על התוצאה.</p>
      <p>לכל כלל שלוש פעולות:</p>
      <ul>
        <li><strong>✅ מאשר</strong> — הכלל נכון. הנתונים לא משתנים, אך השורות
            מסומנות כ״נבדקו על ידי אדם״.</li>
        <li><strong>❌ דוחה</strong> — הכלל שגוי. התיקון מבוטל בכל השורות
            שהכלל יצר, והן חוזרות לבירור ידני.</li>
        <li><strong>✎ שנה</strong> — הכלל בכיוון הנכון אך הערך שגוי.
            הערך החדש מוחל על כל השורות של הכלל בפעולה אחת.</li>
      </ul>
      <p>מדד <strong>הכיסוי</strong> בראש העמוד מראה איזה אחוז מהתיקונים
      מכוסה בכללים שכבר בדקת. זה המספר שקובע כמה אפשר לסמוך על התוצאה —
      לא כמות התיקונים.</p>
    </div>
    <div class="step-card">
      <h4>בדיקת דגימה — שכבה נוספת של ביטחון</h4>
      <p>בדיקת הדגימה בוחנת שורות בודדות ולא כללים. היא מציגה שורה אחת בכל
      פעם — טקסט מקורי מול החלטת המערכת — ומבקשת ״נכון״ או ״שגוי״.
      אם סימנת ״לא בטוח״, השורה <strong>לא</strong> נספרת כתקינה, והשכבה
      מסומנת כלא הושלמה. כמות השורות לבדיקה מוצגת מראש ומשתנה לפי הקובץ.</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(
        '<div class="tip-card"><p>💡 <strong>טיפ:</strong> תשעת הכללים הגדולים '
        'מכסים בדרך כלל כ-90% מכל התיקונים. בדיקה שלהם בלבד — פחות מעשר לחיצות — '
        'נותנת ביטחון ברוב המכריע של הנתונים.</p></div>',
        unsafe_allow_html=True)
